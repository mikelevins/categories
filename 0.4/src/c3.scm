...
(define (c3:good-candidate? reject-lists cand)
  (and (not (some? (lambda (rl) (member cand rl)) 
                  reject-lists))
       cand))

(define (c3:find-candidate input-lists)
  (let ((ins (filter (lambda (i) (not (null? i))) input-lists)))
    (some? (partial c3:good-candidate? (map cdr ins)) (map car ins))))

(define (c3:remove-candidate cand input-lists)
  (map (lambda (ilist) (remove-if (lambda (x)(equal? x cand)) ilist)) input-lists))
