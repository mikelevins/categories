...
;;; galactic alist of closures mapped to their <function>
;;; objects. That way, when you call (add-method! some-fun
;;; some-method), Categories has a way to get hold of the method table
;;; and modify it. This is inefficient, of course, and in an
;;; implementation concerned less about promiscuous portability and
;;; more about performance, it should certainly be done a different
;;; way.

(define fun:*functions* '())

(define (fun:register-function closure function-object)
  (let ((entry (utils:get-entry fun:*functions* closure eq? #f)))
    (if entry
        (set-cdr! entry function-object)
        (set! fun:*functions*
              (cons (cons closure function-object)
                    fun:*functions*)))))

(define (fun:function-object closure)
  (utils:get fun:*functions* closure eq? #f))
