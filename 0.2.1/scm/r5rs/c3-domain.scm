...
;;; according to specificity. When the funciton is applied, we wish to
...
(define (c3:order-methods fun vals)
  (let* ((dom (get-domain fun))
         ...
         (ordered-meths (map (lambda (s)(utils:get entries s c3:signature-equal? #f))
                             ordered-sigs)))
    (utils:filter (lambda (m) m) ordered-meths)))
...
;;; -----------------------------------------------------------------
;;; conveniences: nice surface syntax for -c3-
;;; -----------------------------------------------------------------

(define-syntax c3:method 
  (lambda (form)
    (syntax-case form ()
      ((_  ((arg0 qual0) ...) e0 ...) 
       (syntax (make <method>
                 'signature `(,qual0 ...) 
                 'method-function (lambda (arg0 ...) e0 ...)))))))
