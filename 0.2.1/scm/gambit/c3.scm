...
(define (c3:good-candidate? reject-lists cand)
  (and (not (utils:any? (lambda (rl) (member cand rl)) 
                  reject-lists))
       cand))

(define (c3:find-candidate input-lists)
  (let ((ins (utils:filter (lambda (i) (not (null? i))) input-lists)))
    (utils:any? (utils:partial c3:good-candidate? (map cdr ins)) (map car ins))))

(define (c3:remove-candidate cand input-lists)
  (map (lambda (ilist) (utils:remove cand ilist equal?)) input-lists))

(define (c3:reduce-inputs input-lists output-list)
  (if (utils:every? null? input-lists)
      (reverse output-list)
